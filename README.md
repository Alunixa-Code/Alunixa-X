...
### 动态壁纸（请使用 v1.0.22 或更新版本）

**皮肤管理 → 动态壁纸** 支持上传视频、GIF、APNG 和图片，提供预览、透明度、适配、静音与暂停；v1.0.22 修复大图、视频和场景被宿主安全策略拦截的问题，视频直接使用磁盘后备 File/Blob 播放；也可选择含 `project.json` 的单个 Wallpaper Engine 目录，Video 使用视频播放器，Scene / Web 在 Windows 上使用 Wallpaper Engine 原生窗口渲染喵~

面板的 **修复对话配置** 可定向备份并修复 `features.guardianv2` 类型错误；保存后下次通过 v1.0.22 或更新版本的 Alunixa X 启动时应用壁纸，不打断当前 Codex，完整使用说明与兼容边界见 [动态壁纸说明](docs/wallpapers.md) 喵~
...
使用 v1.0.22 或更新版本，确认启用壁纸且透明度不是零，保存后重新经 Alunixa X 启动；检查“关于”的诊断。管理器预览不是宿主显示成功的证明。
