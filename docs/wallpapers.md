...
本功能从 Alunixa X v1.0.21 起提供，但该版本存在实际宿主显示回归；请使用 v1.0.22 或更新版本，沿用普通图片覆盖层，不替换 DreamSkin 皮肤包喵~
...
4. 下次通过 v1.0.22 或更新版本的 Alunixa X 启动 Codex 时生效，保存不会热注入、替换或重启正在运行的窗口喵~
...
- 开发验收使用独立 Electron 和自有命名 WE 窗口，不对用户当前运行的 Codex 热注入；检查真实画面、动画、捕获清理，不把目录解析或 sourceId 存在当成播放成功喵~
