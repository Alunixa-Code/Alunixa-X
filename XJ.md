...
## 3. Current Status
- 当前开发生图模型配置与 AX 搜索入口，基于 v1.0.18；中断后工作树干净，无残留 Cargo 进程，修改前检查点 `b448544`，本轮尚未发布喵~
...
- 当前本地后续提交仅为 XJ.md/YHYQ.md 验收记录，产品源码、版本清单、测试及发行说明均已在 v1.0.18 标签内推送喵~
...
## 5. Architecture
- 生图配置计划存入既有 SettingsStore 的 imageModels 有序数组，独立 Tauri 命令做脱敏读取、带版本校验的定向保存；复用全量备份，不让普通设置的旧快照覆盖新生图配置喵~
...
## 14. Pending Work
- 当前待办：生图配置后端/保存并发保护、MCP 生成/编辑路由、新栏目与拖拽/默认标记、AX 安装入口兼容、隔离回归与 UI 验收喵~
...
## 20. Next Steps
- 完成本轮功能和隔离验收，更新本文件、YHYQ.md 与 Unreleased 说明并提交；本轮尚未要求新版本 push/发布，不重复发布 v1.0.18 喵~
