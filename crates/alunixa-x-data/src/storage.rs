...
use alunixa_x_core::models::{DeleteResult, DeleteStatus, SessionRef};
...
fn preflight_restore_rows(db: &Connection, tables: &Map<String, Value>) -> anyhow::Result<()> {
    db.execute_batch("SAVEPOINT alunixa_x_restore_preflight")?;
    let restore_result = restore_rows(db, tables);
    let rollback_result = db.execute_batch(
        "ROLLBACK TO alunixa_x_restore_preflight; RELEASE alunixa_x_restore_preflight",
    );
    restore_result?;
    rollback_result?;
    Ok(())
