...
fn default_codex_home_dir() -> PathBuf {
    alunixa_x_core::codex_home::default_codex_home_dir()
}
...
fn provider_sync_db_paths(home: &Path) -> Vec<PathBuf> {
    let mut paths = alunixa_x_core::codex_sqlite::codex_session_db_paths_from_home(home);
    for path in alunixa_x_core::codex_sqlite::codex_thread_reference_db_paths_from_home(home) {
...
fn isolate_stale_lock(path: &Path) -> Option<(ProviderSyncLockOwner, PathBuf)> {
    let owner =
        serde_json::from_slice::<ProviderSyncLockOwner>(&fs::read(path.join("owner.json")).ok()?)
            .ok()?;
    if alunixa_x_core::watcher::process_id_is_running(owner.pid) != Some(false) {
        return None;
    }
...
pub fn preview_session_index_cleanup(
    ...
    let home = codex_home
        ...
        .unwrap_or_else(default_codex_home_dir);
    let sqlite_paths =
        alunixa_x_core::codex_sqlite::codex_thread_reference_db_paths_from_home(&home);
...
pub fn apply_session_index_cleanup(
    ...
    let result = (|| {
        ...
        alunixa_x_core::settings::atomic_write(&plan.path, next_text.as_bytes()).map_err(
            |error| {
                cleanup_apply_error(
                    format!(
...
fn create_backup(
    ...
    for db_path in provider_sync_db_paths(home) {
        for source in alunixa_x_core::codex_sqlite::codex_sqlite_sidecar_paths(&db_path) {
            if !source.exists() {
                continue;
            }
            let relative = alunixa_x_core::codex_sqlite::relative_to_codex_home(home, &source);
    ...
    fs::write(
        ...
        serde_json::to_string_pretty(&json!({
            ...
            "managedBy": "Alunixa X provider sync"
        }))?,
    )?;
    Ok(backup_dir)
}
...
fn prune_backups(home: &Path) -> anyhow::Result<()> {
    ...
    for entry in fs::read_dir(&root)? {
        ...
        if value.get("managedBy").and_then(Value::as_str) == Some("Alunixa X provider sync") {
            managed.push(path);
        }
    }
