...
const IMAGE_REF_PREFIX: &[u8] = b"codex-plus-image-ref:";
...
fn run_rollout_image_cleanup_guarded(
    ...
    for file_plan in &plan.files {
        ...
        write_manifest(&backup_dir, &manifest)?;
        codex_plus_core::settings::atomic_replace_file(&temp_path, &file_plan.path)?;
...
pub fn restore_rollout_image_cleanup(
    ...
    let home = resolve_home(codex_home);
    if !codex_plus_core::watcher::find_session_index_cleanup_blocking_processes().is_empty() {
        return Ok(empty_result(
            &home,
            RolloutImageCleanupStatus::Skipped,
...
mod tests {
    ...
    fn cleanup_only_externalizes_obsolete_checkpoints_and_restores_exact_bytes() {
        ...
        assert!(cleaned.contains("codex-plus-image-ref:"));
