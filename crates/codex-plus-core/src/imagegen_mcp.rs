                ...
                if name != "image_gen" {
                    ...
                    match execute_image_gen(&arguments).await {
                        ...
                        Err(error) => json!({
                            ...
                            "result": {
                                "isError": true,
                                "content": [{ "type": "text", "text": format!("Codex++ image_gen 执行失败：{error:#}") }]
                            }
                        }),
                    }
                }
...
fn image_gen_tool_definition() -> Value {
    json!({
        ...
        "description": "Generate a new raster image or edit local raster images through the active Codex++ provider. Generated files are saved under CODEX_HOME/generated_images.",
        "inputSchema": {
            "type": "object",
            "properties": {
...
fn helper_url() -> anyhow::Result<String> {
    ...
    if !is_loopback || !matches!(url.scheme(), "http" | "https") {
        anyhow::bail!("CODEX_PLUS_HELPER_URL 必须指向本机 HTTP/HTTPS 地址");
    }
...
fn unique_output_path(dir: &Path, stamp: u128, index: usize, extension: &str) -> PathBuf {
    let base = dir.join(format!("codex-plus-{stamp}-{index}.{extension}"));
    if !base.exists() {
        return base;
    }
    for suffix in 2..10_000 {
        let candidate = dir.join(format!("codex-plus-{stamp}-{index}-{suffix}.{extension}"));
        if !candidate.exists() {
            return candidate;
        }
    }
