use alunixa_x_core::assets;
use alunixa_x_core::bridge::{self, BRIDGE_BINDING_NAME};
use alunixa_x_core::cdp::{
...
fn injection_script_prefixes_helper_url_and_sponsor_images() {
    ...
    assert!(script.contains("window.__ALUNIXA_X_SPONSOR_IMAGES__"));
    assert!(script.contains("window.__ALUNIXA_X_VERSION__"));
    assert!(script.contains(alunixa_x_core::version::VERSION));
    assert!(!script.contains("https://discord.gg/y96kX7A76v"));
    assert!(script.contains("data-alunixa-x-discord"));
}
...
fn injection_script_omits_stepwise_runtime_when_disabled() {
    ...
    assert!(script.contains("data-alunixa-x-setting=\"stepwise\""));
}
...
fn pet_real_mouse_settings_are_gated_to_windows_in_injected_ui() {
    ...
    assert!(script.contains("alunixaXIsWindowsPlatform"));
    assert!(script.contains(r#"/\bWindows\b/i.test(navigator.userAgent || "")"#));
    assert!(script.contains("alunixaXIsWindowsPlatform ? `<div"));
}
...
fn pet_real_mouse_script_uses_cdp_push_and_native_avatar_event() {
    ...
    assert!(script.contains("delete window.__alunixaXPetRealMouseLook"));
}
...
fn pet_real_mouse_capability_probe_rejects_v1_without_explicit_v2_evidence() {
    ...
    assert!(!probe.contains("__alunixaXPetRealMouseLook"));
...
fn pet_real_mouse_update_script_stops_when_runtime_capability_is_missing() {
    ...
    assert!(script.contains("__alunixaXPetV2SpriteProbe"));
    assert!(script.contains("updateScreenPoint?.({ x: -125, y: 640 }) === true"));
}

#[test]
fn pet_real_mouse_stop_script_retires_existing_runtime() {
    assert!(assets::pet_real_mouse_stop_script().contains("__alunixaXPetRealMouseLook?.stop?.()"));
}
...
fn injection_script_exposes_image_overlay_config() {
    ...
    assert!(script.contains("window.__ALUNIXA_X_IMAGE_OVERLAY__"));
...
fn injection_script_embeds_explicit_runtime_ports() {
    let settings = alunixa_x_core::settings::BackendSettings::default();
    ...
    assert!(runtime_script.contains("window.__ALUNIXA_X_RUNTIME_DEBUG_PORT__ = 57351;"));
    assert!(runtime_script.contains("http://127.0.0.1:57321"));
    assert!(compatibility_script.contains("window.__ALUNIXA_X_RUNTIME_DEBUG_PORT__ = 0;"));
}
...
fn injection_script_marks_diagnostic_build_and_reports_script_loaded() {
    ...
    assert!(script.contains("window.__ALUNIXA_X_BUILD__"));
    assert!(script.contains(alunixa_x_core::assets::DIAGNOSTIC_BUILD_ID));
    assert!(script.contains("script_loaded"));
    assert!(script.contains("data-alunixa-x-build"));
}
...
fn injection_script_prefers_embedded_local_ads() {
    ...
    assert!(script.contains("window.__ALUNIXA_X_LOCAL_ADS__"));
    assert!(script.contains("normalizeAlunixaXAds(window.__ALUNIXA_X_LOCAL_ADS__)"));
    assert!(script.contains("Date.now()"));
    assert!(!script.contains("BigPizzaV3/Ad-List"));
    assert!(script.contains("9527Code") || script.contains("__ALUNIXA_X_LOCAL_ADS__"));
    assert!(!script.contains("alunixaXAds = normalizeAlunixaXAds(await postJson(\"/ads\", {}));"));
}
...
fn injection_script_polls_verified_bridge_status_without_browser_helper_fetch() {
    ...
    assert!(script.contains("__alunixaXBackendHeartbeat"));
...
fn injection_script_menu_exposes_marketplace_plugin_switch_only() {
    ...
    assert!(script.contains("data-alunixa-x-setting=\"pluginMarketplaceUnlock\""));
    assert!(!script.contains("特殊插件强制安装"));
    assert!(!script.contains("data-alunixa-x-setting=\"forcePluginInstall\""));
    assert!(!script.contains("forcePluginInstall"));
    assert!(!script.contains("强制解锁入口"));
    assert!(!script.contains("data-alunixa-x-setting=\"pluginEntryUnlock\""));
}
...
fn injection_script_defers_backend_mapped_toggles_until_settings_load() {
    ...
    assert!(script.contains("const alunixaXBackendMappedSettings = new Set"));
    assert!(
        script.contains("alunixaXBackendMappedSettings.has(key) && !alunixaXBackendSettingsLoaded")
    );
...
fn injection_script_ignores_stale_backend_settings_responses() {
    ...
    assert!(script.contains("let alunixaXBackendSettingsSeq = 0"));
    assert!(script.contains("const seq = alunixaXBackendSettingsSeq"));
    assert!(script.contains("if (seq !== alunixaXBackendSettingsSeq)"));
    assert!(script.contains("const seq = ++alunixaXBackendSettingsSeq"));
    assert!(script.contains("if (seq === alunixaXBackendSettingsSeq)"));
}
...
fn injection_script_skips_plugin_patch_work_in_relay_mode() {
    ...
    assert!(script.contains("!alunixaXBackendSettingsLoaded"));
...
fn injection_script_disables_plugin_auto_expand_in_relay_mode() {
    ...
    assert!(script.contains("if (pluginPatchDisabledInRelayMode()) return"));
    assert!(script.contains("if (!alunixaXSettings().pluginAutoExpand) return"));
}
...
fn injection_script_keeps_plugin_marketplace_unlock_separate_from_entry_unlock() {
    ...
    assert!(script.contains("if (!alunixaXSettings().pluginMarketplaceUnlock) return"));
...
fn injection_script_keeps_bundled_marketplace_name_for_default_filter() {
    ...
    assert!(script.contains("if (name === \"openai-bundled\") return \"OpenAI插件1(Alunixa X)\""));
}
...
fn injection_script_expands_api_key_plugin_marketplace_requests() {
    ...
    assert!(script.contains("__ALUNIXA_X_PLUGIN_MARKETPLACES__"));
    ...
    assert!(script.contains("if (name === \"openai-curated\") return \"OpenAI插件2(Alunixa X)\""));
    assert!(
        script
            .contains("if (name === \"openai-primary-runtime\") return \"OpenAI插件3(Alunixa X)\"")
    );
    ...
    assert!(script.contains("OpenAI插件1(Alunixa X)"));
    assert!(script.contains("OpenAI插件2(Alunixa X)"));
    assert!(script.contains("OpenAI插件3(Alunixa X)"));
...
fn injection_script_loads_backend_settings_before_initial_scan() {
    ...
    let startup_call = script
        .rfind("window.__alunixaXStartupModelInjection")
...
fn injection_script_exposes_conversation_view_width_control() {
    ...
    assert!(script.contains("data-alunixa-x-conversation-view-width"));
...
fn injection_script_exposes_sub_agent_number_input_hot_reload_and_restart_fallback() {
    ...
    assert!(script.contains("data-alunixa-x-sub-agent-max-threads"));
    ...
    assert!(script.contains("data-alunixa-x-restart-codex"));
...
fn injection_script_exposes_sidebar_thread_id_badge_control() {
    ...
    assert!(script.contains("data-alunixa-x-setting=\"threadIdBadge\""));
...
fn injection_script_unlocks_custom_model_catalog() {
    ...
    assert!(script.contains("applyAlunixaXModelMetadata"));
    ...
    assert!(script.contains("!alunixaXModelNames().length && !codexRetiredManagedModelNames.size"));
    assert!(script.contains("updateCodexManagedModelTracking"));
    assert!(script.contains("alunixaXShouldRemoveRetiredModel"));
    assert!(script.contains("__alunixaXDynamicModelPatchGeneration"));
    assert!(script.contains("codexDynamicModelPatchInstance"));
    assert!(script.contains("client.__alunixaXModelRequestPatch = codexDynamicModelPatchInstance"));
    assert!(script.contains("__alunixaXStartupModelInjection"));
    ...
    assert!(!script.contains("__alunixaXDynamicModelRuntime"));
...
fn injection_script_exposes_fast_service_tier_control() {
    ...
    assert!(script.contains("data-alunixa-x-setting=\"serviceTierControls\""));
...
const store = new Map();
store.set("alunixaXSettings", JSON.stringify({{ serviceTierControls: true }}));
function node() {{
  return {{
    appendChild() {{}},
    prepend() {{}},
...
window.__ALUNIXA_X_TEST_SERVICE_TIER__ = true;
globalThis.document = {{
  scripts: [],
...
require(scriptPath);
const api = window.__alunixaXServiceTierTest;
...
globalThis.window = globalThis;
window.__ALUNIXA_X_TEST_SHARED_TERMINAL__ = true;
...
globalThis.performance = {{ getEntriesByType: () => [] }};
require(scriptPath);
const api = window.__alunixaXSharedTerminalTest;
...
globalThis.window = globalThis;
window.__ALUNIXA_X_TEST_PROJECTLESS__ = true;
window.addEventListener = () => {{}};
window.removeEventListener = () => {{}};
...
store.set("alunixaXSettings", JSON.stringify({{ modelWhitelistUnlock: false }}));
...
require(scriptPath);
void (async () => {{
const api = window.__alunixaXProjectlessTest;
...
#[test]
fn injection_script_refreshes_sidebar_after_delete_undo() {
    let script = assets::injection_script(57321);
    let toast = script
...
fn injection_script_activates_session_delete_once_per_click() {
    ...
    let delegated_delete = script
        ...
        .expect("delete event delegation should end before action helpers")
        .0;
    let action_events = script
        .split_once("function installActionButtonEvents")
        .expect("action button setup should exist")
        .1
    ...
    assert!(!action_events.contains("button.addEventListener(\"pointerup\", onActivate, true);"));
}
...
fn manager_ui_exposes_pure_api_relay_mode_button() {
    let repo = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .expect("core crate should live under crates/alunixa-x-core");
    let source = std::fs::read_to_string(repo.join("apps/alunixa-x-manager/src/App.tsx")).unwrap();
    let commands =
        std::fs::read_to_string(repo.join("apps/alunixa-x-manager/src-tauri/src/lib.rs")).unwrap();
...
fn injection_script_uses_periodic_bridge_heartbeat_instead_of_browser_websocket() {
    ...
    assert!(script.contains("alunixaXBackendStatus = nextStatus;"));
    assert!(script.contains("__alunixaXBackendHeartbeat"));
...
async fn install_bridge_routes_binding_while_waiting_for_command_response() {
    let temp = tempfile::tempdir().unwrap();
    let log_path = temp.path().join("alunixa-x.log");
    alunixa_x_core::diagnostic_log::set_diagnostic_log_path_for_tests(Some(log_path.clone()));
    ...
    alunixa_x_core::diagnostic_log::set_diagnostic_log_path_for_tests(None);
}
...
#[tokio::test]
async fn wallpaper_transport_installs_first_and_survives_backpressure_and_bridge_calls() {
    let tmp = tempfile::tempdir().unwrap();
    let file = tmp.path().join("fixture.png");
    let bytes = vec![73; 9 * 1024 * 1024];
    ...
    let (url, done) = spawn_cdp_server(|mut socket| async move {
        let enable = recv_json(&mut socket).await;
        assert_eq!(enable["method"], "Fetch.enable");
        assert_eq!(
            enable["params"]["patterns"],
            json!([{
                "urlPattern":"app://-/_alunixa-x-wallpaper/*","requestStage":"Request"
            }])
        );
        send_json(&mut socket, json!({"id":enable["id"],"result":{}})).await;
        for expected_id in 1..=5 {
            let command = recv_json(&mut socket).await;
            assert_eq!(command["id"], expected_id);
            if expected_id == 5 {
                // An awaited fetch during installation must not deadlock.
                send_json(
                    &mut socket,
                    json!({"method":"Fetch.requestPaused","params":{
                        "requestId":"during-install","request":{"method":"HEAD","headers":{},
                            "url":"app://-/_alunixa-x-wallpaper/media"}
                    }}),
                )
                .await;
                let response = recv_json(&mut socket).await;
                assert_eq!(response["method"], "Fetch.fulfillRequest");
                assert_eq!(response["params"]["responseCode"], 200);
                assert_eq!(response["params"]["body"], "");
                send_json(&mut socket, json!({"id":response["id"],"result":{}})).await;
            }
            send_json(&mut socket, json!({"id":expected_id,"result":{}})).await;
        }
        send_json(
            &mut socket,
            json!({"method":"Fetch.requestPaused","params":{
                "requestId":"large-body","request":{"method":"GET","headers":{},
                    "url":"app://-/_alunixa-x-wallpaper/media"}
            }}),
        )
        .await;
        // Exceed the bridge's 250ms generation tick while the sender is busy.
        tokio::time::sleep(Duration::from_millis(650)).await;
        let response = recv_json(&mut socket).await;
        assert_eq!(response["method"], "Fetch.fulfillRequest");
        assert_eq!(response["params"]["requestId"], "large-body");
        assert_eq!(
            base64::engine::general_purpose::STANDARD
                .decode(response["params"]["body"].as_str().unwrap())
                .unwrap(),
            vec![73; 9 * 1024 * 1024]
        );
        send_json(&mut socket, json!({"id":response["id"],"result":{}})).await;
        send_json(
            ...
            json!({"method":"Runtime.bindingCalled","params":{
                "payload":json!({"id":"after-media","path":"fixture","payload":{}}).to_string()
            }}),
        )
        .await;
        let response = recv_json(&mut socket).await;
        assert_expression_contains_request(&response, "after-media");
        close_socket(&mut socket).await;
    })
    .await;
    ...
    tokio::time::timeout(Duration::from_secs(15), done)
        ...
        .unwrap()
        .unwrap();
    let reason = disconnected.await.unwrap();
    assert!(reason.contains("CDP websocket"), "{reason}");
}
