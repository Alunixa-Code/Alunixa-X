use alunixa_x_core::assets::{force_chinese_locale_config, injection_script_with_settings};
...
fn injection_script_includes_force_chinese_locale_global_and_patch() {
    ...
    assert!(script.contains(
        "window.__ALUNIXA_X_FORCE_CHINESE_LOCALE__ = {\"enabled\":true,\"locale\":\"zh-CN\"};"
    ));
    assert!(script.contains(
    ...
    assert!(script.contains("alunixaX.forceChineseLocale.managed.v1"));
    assert!(!script.contains("setItem(\"localeOverride\""));

    settings.codex_app_force_chinese_locale = false;
    let script = injection_script_with_settings(0, &settings);
    assert!(script.contains(
        "window.__ALUNIXA_X_FORCE_CHINESE_LOCALE__ = {\"enabled\":false,\"locale\":\"zh-CN\"};"
    ));
}
