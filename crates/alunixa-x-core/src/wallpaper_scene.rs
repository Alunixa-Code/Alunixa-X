...
pub fn state(project: &Path) -> Value {
    match snapshot().lock() {
        ...
        _ => json!({"status":"failed","message":"场景尚未启动，请保存设置并通过 Alunixa X 重新启动 Codex"}),
    }
}
...
pub fn engine_path(project: &Path, configured: &str) -> anyhow::Result<PathBuf> {
    if !cfg!(windows) { bail!("Wallpaper Engine 原生 Scene 仅支持 Windows；其他平台可使用视频或 Web 壁纸"); }
    let valid = |path: &Path| path.is_file() && path.file_name().and_then(|s| s.to_str())
        .is_some_and(|s| matches!(s.to_ascii_lowercase().as_str(), "wallpaper64.exe" | "wallpaper32.exe"));
    if !configured.trim().is_empty() {
        let path = PathBuf::from(configured.trim());
        if valid(&path) { return Ok(path.canonicalize()?); }
    ...
    for root in project.ancestors() {
        for relative in ["wallpaper64.exe", "wallpaper32.exe", "common/wallpaper_engine/wallpaper64.exe", "common/wallpaper_engine/wallpaper32.exe"] {
            let candidate = root.join(relative);
            if valid(&candidate) { return Ok(candidate.canonicalize()?); }
        }
    }
...
pub fn start(inspector_port: u16, settings: &BackendSettings) {
    ...
    tokio::spawn(async move {
        let Ok(source) = crate::wallpaper::resolve(Path::new(settings.codex_app_image_overlay_path.trim())) else { return };
        if let Ok(mut state) = snapshot().lock() { *state = (source.path.clone(), json!({"status":"waiting"})); }
        let result = prepare(inspector_port, &source.path, &settings).await;
        let value = match result {
            Ok(value) => value,
            Err(error) => json!({"status":"failed","message":error.to_string()}),
        };
        if let Ok(mut state) = snapshot().lock() { *state = (source.path, value); }
    });
}
...
async fn prepare(port: u16, project: &Path, settings: &BackendSettings) -> anyhow::Result<Value> {
    ...
    for _ in 0..20 {
        if let Ok(targets) = crate::cdp::list_targets(port).await {
            target = targets.into_iter().find(|target| target.target_type == "node" && target.web_socket_debugger_url.is_some());
    ...
    let result = crate::bridge::evaluate_script_with_await_promise(
        target.web_socket_debugger_url.as_deref().unwrap(), &script, true,
    ).await?;
    let text = result.pointer("/result/result/value").or_else(|| result.pointer("/result/value"))
        .and_then(Value::as_str).context("原生场景返回了无效捕获结果")?;
...
pub fn capture_script(engine: &Path, project: &Path, title: &str, muted: bool) -> String {
    ...
    r#"(async () => {
      ...
      const run = (args) => new Promise((resolve,reject) => {
        ...
        child.once("exit",code=>code === 0 ? resolve() : reject(new Error("Wallpaper Engine command failed")));
      });
...
fn send_window_behind(title: &str) {
    ...
    use windows::Win32::UI::WindowsAndMessaging::{FindWindowW, SetWindowPos, HWND_BOTTOM, SWP_NOACTIVATE, SWP_NOMOVE, SWP_NOSIZE};
    let title: Vec<u16> = title.encode_utf16().chain(Some(0)).collect();
    unsafe {
        if let Ok(window) = FindWindowW(PCWSTR::null(), PCWSTR(title.as_ptr())) {
            let _ = SetWindowPos(window, HWND_BOTTOM, 0,0,0,0, SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOSIZE);
        }
    }
}
#[cfg(not(windows))]
...
mod tests {
    ...
    fn scene_never_controls_desktop_or_launches_project_programs() {
        let script = capture_script(Path::new("wallpaper64.exe"),Path::new("project.json"),"unique fixture",true);
