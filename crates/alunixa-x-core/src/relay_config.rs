...
pub fn set_codex_imagegen_mcp_in_home(
    ...
    if enabled {
        let command = imagegen_mcp_path
            ...
            .context("Alunixa X image_gen MCP 路径无效")?;
...
pub async fn test_relay_profile(
    ...
    let client = crate::http_client::proxied_client("AlunixaX/RelayTest")?;
...
fn live_external_model_catalog(home: &Path) -> Option<String> {
    ...
    (!path.is_empty()
        && !is_alunixa_x_managed_model_catalog(home, path)
...
fn is_alunixa_x_managed_model_catalog(home: &Path, path: &str) -> bool {
...
pub fn relay_profile_api_key(profile: &RelayProfile) -> String {
    if profile.relay_mode == crate::settings::RelayMode::Aggregate {
        return "alunixa-x-aggregate".to_string();
    }
    if profile.relay_mode == crate::settings::RelayMode::CustomModels {
        return "alunixa-x-custom".to_string();
    }
...
pub fn normalize_relay_profile_for_storage(profile: &mut RelayProfile) -> anyhow::Result<()> {
    ...
    if profile.relay_mode == crate::settings::RelayMode::CustomModels {
        ...
        profile.auth_contents = serde_json::to_string_pretty(&json!({
            "auth_mode": "apikey",
            "OPENAI_API_KEY": "alunixa-x-custom"
        }))?;
...
fn create_live_backup(
    ...
    let backup_dir = home
        .join("backups")
        .join(format!("alunixa-x-live-{}", timestamp_millis()));
