...
fn repair_scope(scope: &mut dyn TableLike, prefix: &str, changes: &mut Vec<String>) {
    ...
    let Some(guardian) = features.get_mut("guardianv2") else {
        return;
    };
...
fn valid_field(kind: &str, key: &str, item: &Item) -> bool {
    match (kind, key) {
        ...
        ("guardian", "reasoning_effort") => item.as_str().is_some_and(|v| !v.is_empty()),
        (
            "guardian",
            "max_action_tokens"
        ...
        | (
            ...
            .as_integer()
            .is_some_and(|v| (100..=100_000).contains(&v)),
        ...
        ("guardian", "review_threshold") => item
            .as_float()
            .or_else(|| item.as_integer().map(|v| v as f64))
            .is_some_and(|v| v.is_finite() && (0.0..=1.0).contains(&v)),
        ("transcript", "sources") => item.as_array().is_some_and(|values| {
            values.iter().all(|v| {
                v.as_str()
                    .is_some_and(|v| matches!(v, "tool_calls" | "tool_outputs" | "reasoning"))
            })
        }),
        _ => false,
    }
...
mod tests {
    ...
    fn repairs_invalid_nested_fields_without_disabling_guardian() {
        ...
        assert_eq!(
            v["profiles"]["work"]["features"]["guardianv2"]["enabled"].as_bool(),
            Some(false)
        );
        assert_eq!(
            v["features"]["guardianv2"]["transcript"]["include_images"].as_bool(),
            Some(false)
        );
    ...
    fn valid_boolean_and_structures_are_byte_preserved() {
        for value in [
            "true",
            "false",
            "{enabled=true,thread_context=true}",
            "{transcript={include_images=false,sources=['reasoning']}, review_threshold=0.5}",
            "{}",
        ] {
