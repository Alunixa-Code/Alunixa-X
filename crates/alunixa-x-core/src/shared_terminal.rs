...
pub fn proxy_command(
    ...
    Some(format!(
        "{executable} --alunixa-x-shared-terminal --request {} --thread {} --cwd {} --command {}",
        encode(request_id),
        encode(thread_id),
        encode(cwd),
        encode(command)
...
pub async fn run_proxy(request: SharedTerminalRequest) -> anyhow::Result<SharedTerminalResult> {
    let status = StatusStore::default()
        .load_latest()?
        .context("未找到 Alunixa X 启动状态，请用 Alunixa X 启动器重新打开 Codex")?;
    if !matches!(status.status.as_str(), "running" | "running_degraded") {
        anyhow::bail!("Alunixa X 后端未运行，请用 Alunixa X 启动器重新打开 Codex");
    }
    let helper_port = status.helper_port.context("Alunixa X Helper 端口不可用")?;
    ...
    let response = client
        ...
        .send()
        .await
        .context("连接 Alunixa X 共享终端失败")?;
    let status_code = response.status();
    let body = response
        .bytes()
        .await
        .context("读取 Alunixa X 共享终端响应失败")?;
    ...
    serde_json::from_slice(&body).context("解析 Alunixa X 共享终端响应失败")
}
