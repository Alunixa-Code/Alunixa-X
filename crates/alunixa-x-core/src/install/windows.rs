...
const UNINSTALL_SUBKEY: &str = r"Software\Microsoft\Windows\CurrentVersion\Uninstall\AlunixaX";
const LEGACY_UNINSTALL_SUBKEY: &str =
    r"Software\Microsoft\Windows\CurrentVersion\Uninstall\AlunixaXLegacy";
const URL_PROTOCOL_SUBKEY: &str = r"Software\Classes\alunixax";
...
pub fn uninstall_shortcuts(options: &InstallOptions) -> anyhow::Result<()> {
    ...
    for key in [
        format!(r"{LEGACY_URL_PROTOCOL_SUBKEY}\shell\open\command"),
        format!(r"{LEGACY_URL_PROTOCOL_SUBKEY}\shell\open"),
        format!(r"{LEGACY_URL_PROTOCOL_SUBKEY}\shell"),
        LEGACY_URL_PROTOCOL_SUBKEY.to_string(),
    ] {
...
fn write_uninstall_registration(plan: &WindowsEntrypointPlan) -> anyhow::Result<()> {
    ...
    let install_location = Path::new(&plan.manager_path)
        ...
        .to_string();
    for (name, value) in [
        ("DisplayName", "Alunixa X".to_string()),
        ("DisplayVersion", crate::version::VERSION.to_string()),
        ("Publisher", "Alunixa-Code".to_string()),
...
fn default_icon_path() -> PathBuf {
    std::env::current_exe()
        ...
        .map(|path| path.join("alunixa-x.ico"))
        .unwrap_or_else(|| PathBuf::from("alunixa-x.ico"))
}
