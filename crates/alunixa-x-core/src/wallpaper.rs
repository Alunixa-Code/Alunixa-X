...
pub async fn cdp_response(request: &Value, settings: &crate::settings::BackendSettings) -> Value {
    ...
    let response = async {
        ...
        let relative = url
            .strip_prefix(APP_RESOURCE_BASE)
            .context("invalid wallpaper URL")?;
        let method = request["request"]["method"].as_str().unwrap_or("GET");
        let range = request["request"]["headers"]
            .as_object()
            .and_then(|headers| {
                headers
                    .iter()
        ...
        let bytes = match &mut response.body {
            ...
            Body::File { file, start, count } => {
                ...
                anyhow::ensure!(
                    ...
                    "wallpaper file changed during read"
                );
                bytes
            }
        };
...
async fn prepare_response(
    ...
    if method == "OPTIONS" {
        return Ok(ResourceResponse::empty(204));
    }
    ...
    let Ok(source) = result else {
        return Ok(ResourceResponse::empty(404));
    };
