...
pub fn resolve(path: &Path) -> anyhow::Result<WallpaperSource> {
    if path.as_os_str().is_empty() { bail!("请选择壁纸文件或项目目录"); }
    let selected = if path.is_dir() { path.join("project.json") } else { path.to_owned() };
    if media_type(&selected).is_some() {
        ...
        return Ok(WallpaperSource {
            ...
            title: entry.file_name().unwrap_or_default().to_string_lossy().into(),
            path: entry.clone(),
    ...
    let value: Value = serde_json::from_slice(&std::fs::read(&project)?).context("project.json 格式无效")?;
    ...
    let actual_kind = match kind.as_str() {
        ...
        "web" if entry.extension().and_then(|s| s.to_str()).is_some_and(|s| s.eq_ignore_ascii_case("html") || s.eq_ignore_ascii_case("htm")) => "web",
        "scene" => "scene",
        _ => bail!("不支持此 Wallpaper Engine 项目类型或入口文件；不执行 Application 类型壁纸"),
    };
    Ok(WallpaperSource {
        kind: actual_kind.into(),
        title: value["title"].as_str().unwrap_or("Wallpaper Engine").chars().take(160).collect(),
        path: project,
        entry,
        root,
    })
...
pub fn contained_file(root: &Path, relative: &str) -> anyhow::Result<PathBuf> {
    ...
    if path.is_absolute() || path.components().any(|part| !matches!(part, Component::Normal(_))) || relative.contains(':') {
    ...
    if !canonical.starts_with(&canonical_root) { bail!("项目资源不在所选壁纸目录内"); }
    Ok(canonical)
}

pub fn import_media(source: &Path, state_dir: &Path) -> anyhow::Result<WallpaperSource> {
    let resolved = resolve(source)?;
    if media_type(source).is_none() { bail!("项目目录请使用选择目录，不需要上传"); }
    let directory = state_dir.join("wallpapers");
    std::fs::create_dir_all(&directory)?;
    let mut temp = tempfile::NamedTempFile::new_in(&directory)?;
    let mut input = std::fs::File::open(&resolved.entry)?;
    std::io::copy(&mut input, &mut temp)?;
    temp.as_file().sync_all()?;
    let ext = source.extension().context("壁纸扩展名无效")?.to_string_lossy().to_ascii_lowercase();
...
pub fn runtime_config(helper_port: u16, settings: &crate::settings::BackendSettings) -> Value {
    let resolved = settings.codex_app_image_overlay_enabled
    ...
    if let Some(result) = resolved {
        match result {
            Ok(source) => {
                let version = std::fs::metadata(&source.entry).ok().and_then(|m| m.modified().ok())
                    .and_then(|t| t.duration_since(std::time::UNIX_EPOCH).ok()).map(|t| t.as_nanos()).unwrap_or(0);
                let relative = source.entry.strip_prefix(&source.root).unwrap_or(Path::new(""));
                let web_path = relative.components().map(|s|
                    url::form_urlencoded::byte_serialize(s.as_os_str().to_string_lossy().as_bytes()).collect::<String>()
                ).collect::<Vec<_>>().join("/");
                ...
                config["sceneUrl"] = json!(format!("http://127.0.0.1:{helper_port}/wallpaper/scene"));
            }
            Err(_) => config["error"] = json!("壁纸文件或项目入口不可用，请在 Alunixa X 中重新选择"),
        }
    }
    config
}
...
pub fn byte_range(header: Option<&str>, length: u64) -> anyhow::Result<(u64, u64, bool)> {
    if length == 0 { bail!("empty file"); }
    let Some(header) = header else { return Ok((0, length - 1, false)) };
    let range = header.strip_prefix("bytes=").context("invalid range unit")?;
    let (start, end) = range.split_once('-').context("invalid range")?;
    if range.contains(',') { bail!("multiple ranges unsupported"); }
    let (start, end) = if start.is_empty() {
        let suffix: u64 = end.parse()?;
        if suffix == 0 { bail!("empty suffix"); }
    ...
    if start > end || start >= length { bail!("unsatisfiable range"); }
    Ok((start, end, true))
}

pub async fn serve(
    stream: &mut tokio::net::TcpStream, method: &str, raw_path: &str, range: Option<&str>,
    ...
    let Ok(source) = result else { return error_response(stream, "404 Not Found", "").await };
    if path == "/wallpaper/scene" {
        let state = crate::wallpaper_scene::state(&source.path);
        let bytes = serde_json::to_vec(&state)?;
        let head = format!("HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nContent-Length: {}\r\nAccess-Control-Allow-Origin: *\r\nCache-Control: no-store\r\nConnection: close\r\n\r\n", bytes.len());
        stream.write_all(head.as_bytes()).await?;
        if method != "HEAD" { stream.write_all(&bytes).await?; }
        return Ok(());
    }
    let file = if path == "/wallpaper/media" && matches!(source.kind.as_str(), "image" | "video") {
        ...
        let relative = match url::form_urlencoded::parse(format!("p={}", encoded.replace('+', "%2B")).as_bytes()).next() {
    ...
    } else { return error_response(stream, "404 Not Found", "").await };
    let mime = media_type(&file).map(|(_, mime)| mime).or_else(|| match file.extension()?.to_str()?.to_ascii_lowercase().as_str() {
    ...
    let Some(mime) = mime else { return error_response(stream, "404 Not Found", "").await };
    let mut file = tokio::fs::File::open(file).await?;
    let length = file.metadata().await?.len();
    let (start, end, partial) = match byte_range(range, length) {
        Ok(range) => range,
        Err(_) => return error_response(stream, "416 Range Not Satisfiable", &format!("Content-Range: bytes */{length}\r\n")).await,
    };
    ...
    let head = format!(
        ...
        if partial { "206 Partial Content" } else { "200 OK" }, end-start+1,
        if partial { format!("Content-Range: bytes {start}-{end}/{length}\r\n") } else { String::new() },
    );
    stream.write_all(head.as_bytes()).await?;
    if method != "HEAD" {
...
async fn error_response(stream: &mut tokio::net::TcpStream, status: &str, extra: &str) -> anyhow::Result<()> {
...
mod tests {
    ...
    fn range_contract() {
        ...
        for range in ["bytes=100-", "bytes=-0", "bytes=8-4", "bytes=0-1,3-5", "wat=1-2"] {
    ...
    fn resolves_projects_but_rejects_escape_and_application() {
        ...
        for (kind,file,valid) in [("video","video.mp4",true),("video","../secret.mp4",false),("application","video.mp4",false),("scene","video.mp4",true)] {
            std::fs::write(root.join("project.json"), serde_json::to_vec(&json!({"type":kind,"file":file,"title":"测试"})).unwrap()).unwrap();
