...
pub const WATCHER_RUN_NAME: &str = "AlunixaXWatcher";
pub const WATCHER_RUN_KEY: &str = r"Software\Microsoft\Windows\CurrentVersion\Run";
pub const WATCHER_STARTUP_SHORTCUT_NAME: &str = "AlunixaXWatcher.lnk";
...
pub fn filter_killable_launcher_processes<'a>(
    ...
    processes
        ...
        .filter(|(process_id, _, exe_file)| {
            !protected.contains(process_id) && exe_file.eq_ignore_ascii_case("alunixa-x.exe")
        })
...
pub fn stop_launcher_processes_and_wait() -> anyhow::Result<()> {
    ...
    loop {
        ...
        if std::time::Instant::now() >= deadline {
            anyhow::bail!(
                "Alunixa X 后台进程未能在 {} 毫秒内退出，剩余 PID：{}",
...
fn create_startup_shortcut(launcher_path: &Path, arguments: &str) -> anyhow::Result<()> {
    ...
    crate::windows_integration::create_shortcut(&crate::windows_integration::ShortcutSpec {
        ...
        description: "Alunixa X watcher".to_string(),
        icon: None,
        show_minimized: true,
    })
}
