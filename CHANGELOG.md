...
- 修复正式数据启动器漏接壁纸路由、返回 `Unknown bridge path`；两个正式启动器和实播fixture改用同一renderer bridge安装入口，补真实WebSocket分发/禁用保护/普通路由共存和入口契约回归喵~
- Wallpaper Engine Scene/Web从创建时在所有显示器之外渲染，移除任务栏/边框、不激活/不最小化，不再用“置底”冒充隐藏；初始化状态同步发布，离屏布置失败只关闭自有窗口喵~
- 真实Electron实播门禁增加Win32离屏坐标/任务栏/焦点/最小化检查，同时保留静态/大图/动画/视频/原生非黑与变化帧验证；不再让测试独立接线掩盖正式路径遗漏喵~
- 中英俄README/壁纸指南说明实际根因、升级生效方式和原图片背景的独立使用路径，保留俄语及guardianv2修复，不改第三方依赖喵~
