...
- 优化低频界面资源组织、按需加载和运行时生命周期，进一步减少主界面初始负担喵~
- 优化管理器细节交互、全屏状态切换与键盘操作体验，补充窄窗布局和减少动效偏好适配喵~
- 优化非日常界面的按需加载与本机轻量状态恢复，减少主界面初始加载负担喵~

## 1.0.25 - 2026-09-25

- 修复 Codex 26.917 更新后多模型上下文窗口不一致：根级 `model_context_window` 和 `model_auto_compact_token_limit` 会全局覆盖模型目录，导致切换到大窗口模型后仍沿用启动模型的较小窗口喵~
- 自定义多模型模式现在只在 `model_catalog_json` 中逐模型保存窗口与自动压缩阈值，保存、切换、规范化和启动审计均移除旧根级覆盖；普通单模型、纯 API 等模式保持原有根配置行为喵~
- 管理器配置预览同步移除多模型根级窗口键，同时保留嵌套 `[profiles.*]` 用户配置；更新中文、英文、俄语说明和回归测试，明确模型目录为多模型模式的权威来源喵~
- 使用当前 Codex 26.917.9434.0 app-server 完成正反契约：根覆盖时 Sol 被限制为 272000，移除后恢复目录中的 1050000；本轮同时确认现有视频背景在当前实例真实播放，不重写已通过的媒体链路喵~
- 发布前本地 workspace 1128/0/1、前端 118/118、全目标编译、TypeScript、Vite、中英俄词典、品牌和格式门禁全部通过；第三方依赖不升级，正式安装资产仍由唯一三平台 GitHub Actions 构建喵~

## 1.0.24 - 2026-09-16

- 修复正式数据启动器漏接媒体路由而返回 `Unknown bridge path`；正式两种启动器和实播fixture共用同一renderer bridge入口，覆盖大图、视频与旧项目预览喵~
- 按用户稳定优先的反馈，移除Wallpaper Engine原生Scene/Web启动、额外窗口、窗口捕获和项目脚本服务；离屏黑屏实验已撤下，不再尝试启动引擎喵~
- 保留普通图片、GIF/APNG、视频的导入、预览、透明度、适配、静音和暂停；移除目录/引擎选择UI，旧Scene/Web仅显示目录内安全预览图片并明确标记，无预览/路径越界/非图片提示重新选择喵~
- 保留俄语与guardianv2修复，更新完整中英俄指南；新增生产分发、原生移除、旧配置静态降级和路径边界回归，修复Electron验证失败仍退出0的缺陷，不升级第三方依赖喵~

## 1.0.23 - 2026-09-16

- 新增完整俄语管理器词典：923 条界面文案、80 条动态模板、92 条既有后端消息与 79 条动态错误规则；覆盖供应商、生图模型、壁纸、任务、工具与维护界面喵~
- 右上角改为简体中文/English/Русский 明确选择，持久化后统一重载；增加切换确认、存储失败保留编辑、三语托盘与日期/HTML lang，保持 Codex 原生语言开关独立喵~
- 适配俄语导航长文案换行，概览运行状态本地化，Windows NSIS 增加 Russian 标准页资源；保留内置离线西里尔字母字体，不引入在线翻译或新依赖喵~
- 新增详细 README_RU.md 与真实隔离俄语界面截图，扩展中文/英文介绍的安装、首次配置、协议、模型、生图、壁纸矩阵、功能分区、备份回退、数据目录、FAQ 和开发指引，三语互链喵~
- i18n 门禁改为扫描全部生产 TS/TSX，并检查缺漏/陈旧键、模板参数、后端正则及俄语中文回退；新增 11 项语言单测和生产 Chromium 的三语切换/10页面/深浅主题/窄窗验证喵~
- 保留 v1.0.22 壁纸播放链路与 guardianv2 修复，不修改真实配置或运行中的 Codex；新标签经三平台 GitHub Actions 全量构建后发布六项安装资产，不覆盖历史标签喵~
...
- 新增媒体流、真实 Chromium 播放/动画/隔离、管理器交互、原生场景捕获脚本与本机 CLI 临时配置解析回归；使用说明与原生场景验收边界见 `docs/wallpapers.md`，由 GitHub Actions 三平台构建发布，不替换正在运行的实例喵~

## 1.0.19 - 2026-09-14

- 左侧新增“生图模型”，支持多组 API/Key/Model 增删改、拖拽和键盘排序、上下移动，首项自动作为默认并显示“默认”标记，排序自动保存喵~
- 图片生成与编辑 MCP 每次调用重读独立配置，默认首项、可显式指定 model/profile_id；无需修改对话供应商，清空全部后恢复原 Helper 路由，不进行跨生图配置重试或轮换喵~
- 独立配置使用脱敏读取、Key 留空保留、并发修订校验和定向保存；普通设置旧快照不能覆盖生图顺序，完整备份保留配置与 Key，普通 UI 回包和诊断报告不返回生图 Key 喵~
- Windows 安装器与安装维护增加 AX 开始菜单快捷方式；macOS bundle 文件名及显示名包含 (AX)，支持旧名/混合安装，优先按已解析的同级应用路径启动，保持 bundle ID 不变喵~
- 修复 macOS 安装维护遗漏 imagegen MCP companion，写入前检查必要程序并保留可执行权限；下载错误不回显签名 URL，不将重定向错误体保存为图片喵~
- 新增生图存储/并发/备份、MCP 真实本机 HTTP 路由/凭据隔离、跨平台安装和生产 UI 拖拽回归；保留 v1.0.18 的协议保真修复与三平台完整发布门禁喵~

## 1.0.18 - 2026-09-14

- 修复本地协议转换把孤立工具执行结果降为用户消息、跳过非法历史项、重写非法函数参数的问题；不支持的语义返回明确的协议错误，不再伪装成功喵~
- 修复 SSE 尾帧/用量丢失、CR/LF 和 UTF-8 分片、损坏 JSON 静默跳过、重复终态、截断响应错发 completed；补齐事件序号、refusal、自定义工具结束事件、分片函数名和执行前完整性校验喵~
- 思考与正文保持独立输出项，流式 `<think>` 内容及时发往思考通道，不再等待完整结束标签；未闭合思考块不写入正文喵~
- Anthropic thinking/signature/redacted_thinking 与 Gemini thoughtSignature/函数原始 ID 使用自包含原生回放项，不依赖进程内全局缓存；缺失、被编辑或跨协议的回放数据明确拒绝喵~
- 修复跨请求工具/响应 ID 冲突、缓存 token 重复扣减、图片 detail 和结构化输出字段丢失；原生思考参数显式映射或拒绝，不再静默忽略喵~
- 发送结果不明的断连、超时和服务端错误不自动切换供应商重放；仅连接建立失败或明确认证/限流拒绝可进行有界重试/切换，已输出内容后不再重试 Responses ID 协商喵~
- 上游忽略 stream 返回 JSON 时保持下游 SSE 契约；已结束生成却不关闭流的等待有独立截止时间，异常使用协议错误而不是注入对话正文喵~
- 新增协议保真回归 31 项，既有协议回归 71 项，共 102 项通过；正式安装包由三平台 GitHub Actions 全套测试与构建后发布，不覆盖 v1.0.17 资产、不热替换运行中的 Codex，能力边界见 `docs/protocol-fidelity.md` 喵~

## 1.0.17 - 2026-09-13

- 修复自定义多模型供应商的 config.toml 预览使用过时汇总字段：现按明确选中的启动模型生成上下文窗口与压缩阈值，通用配置合并后再应用模型设定，避免预览和实际写入不一致喵~
- 启动模型改为可显式选择，并说明根配置只跟随启动模型、其他模型设置写入模型目录；编辑其他模型不自动切换启动模型或打断当前任务喵~
- 修复清空自定义模型窗口后旧 root 覆盖仍残留、关闭压缩后汇总值未清理，以及 K/M 单位在校验与实际写入中解析不一致；支持 1M、1.05M 等输入，检查数值溢出喵~
- 修复带注释 TOML 表头导致窗口预览/表单写入误修改嵌套 profile 的问题，显式保存的窗口只更新根级托管键，保留其他 profile 和外部提示词喵~
- 供应商保存后与启动前回读校验窗口/压缩阈值是否匹配启动模型，不再仅以文件写入成功作为上下文配置成功的依据；失败沿用原子切换回滚喵~
- 新增完整保存链路、非启动模型目录更新、清空/禁用、单位边界、预览与回读失配回归，保留已有配置保护，不把“编辑其他模型没有改根配置”误判为保存丢失喵~
...
- 高级提示词启动检查现在明确拒绝非字符串引用、缺失路径、目录/非普通文件、空内容、非 UTF-8、超限文件和 NUL 字符；托管提示词可从正文、`.last-good` 或模板恢复，外部用户文件只读检查不覆盖喵~
- 新增 Fast 开关、inline TOML `features` 表、关闭清理、供应商写入保留、启动审计异常和高级提示词内容校验回归；前端与 Rust 工作区全量测试通过喵~

## 1.0.15 - 2026-09-11

- 新增 Alunixa X 启动前全面配置审计，在真正启动 Codex 或重新激活已有实例前统一检查 `config.toml`、`auth.json`、`hooks.json`、保存的供应商配置片段、退役字段、线程上限、image_gen MCP、WSS/provider 状态和高级提示词文件喵~
- 启动审计只修复 Alunixa X 自己拥有的残留项，保留用户自建 Provider、凭据、外部 hooks、外部高级提示词和第三方插件市场；检测到无法安全修复或配置失配时在启动 Codex 前失败，不带错启动喵~
- 关闭 Computer Use 或托管插件市场后自动删除仅由 Alunixa X 写入的配置和插件条目，同时保留用户自建条目；保留现有图片生成、Fast、中文界面、供应商切换和高级提示词恢复行为喵~
- 高级提示词启动扫描检查引用路径、普通文件属性、UTF-8、大小限制、NUL 字符、托管文件和 `.last-good` 备份，并继续避免用设置保存值覆盖用户已有正文喵~
- 新增启动审计、配置清理、关闭开关残留、启动器首次启动和已有实例重新激活路径回归；正式发行继续由三平台 GitHub Actions 完整测试、构建和六项安装资产校验喵~

## 1.0.14 - 2026-09-11

- 按用户要求移除“实验性上下文”功能，而不是继续修补或默认关闭：删除管理器入口、设置字段、本地上下文 MCP 实现、窗口切换提示生成及对应旧专项验证脚本喵~
- 清理 `features.token_budget`、`features.context_management.experimental_mode` 和 `mcp_servers.alunixa-x-context`；删除后不再注入要求模型保存笔记、读取历史或主动切换上下文窗口的提示喵~
- 管理器加载/保存、导入配置、切换供应商及启动前执行仅删除的升级迁移，同时清理公共配置与供应商快照，防止旧快照把该功能重新写回喵~
- 真实文件修改前保存本地原始备份，写入前检查并发变化；保留已存笔记、任务历史、其他 MCP、Provider、凭据、模型上下文长度及普通自动压缩阈值喵~
- 保留 Fast 按钮、中文界面、高级提示词恢复和图片生成 MCP；旧 companion 上下文参数不再启动任何上下文服务喵~
- 新增退役迁移、配置保留、重复执行、旧字段拒绝和供应商切换回归，三平台完整测试与六资产发行门禁保持启用；移除已退役功能专用的官方 CLI 下载与窗口切换测试喵~

## 1.0.13 - 2026-09-10

- 修复新版 Codex 输入栏找不到 Fast 控件挂载位置：统一识别旧 `.composer-footer`、新版 CSS 模块 footer 和 `data-composer-footer-responsive`，不绑定某次构建的哈希类名喵~
- 优先选择实际输入栏底部而不是包含侧栏/其他工具栏的大范围祖先；排除隐藏、inert 和已断开节点，兼容紧凑布局与输入栏重新挂载喵~
- 防止 Fast 控件嵌入原生按钮，重复扫描复用单个控件和事件处理器，避免把控件插到自身前面；新版 footer 的 DOM 变化也能触发已有有界刷新流程喵~
- 为未连接状态补充 `aria-disabled`，保留用户显式关闭设置、已有模型能力判定和服务等级请求策略，不强制开启 Fast 或为不支持的模型设置 priority 喵~
- 新增十一项隔离前端行为回归及一项 Rust 资源嵌入契约，前端总计 77 项；正式安装文件由三平台完整测试通过后的 GitHub Actions 构建发布喵~
- 本版没有实现 Ultra“确认一次后记住”，没有修改原生完全访问、Guardian、取消或受限模式确认，发行范围只包含 Fast 布局兼容修复喵~

## 1.0.12 - 2026-09-06

- 修复部分 Codex 已选择中文但页面仍为英文：同时兼容新版 Statsig `getLayer` 与旧版 `getDynamicConfig` 语言入口，只调整内置语言开关，保留其他实验和能力配置喵~
- 兼容冻结的语言配置对象、延迟注册的客户端与慢启动桥接；设置请求增加有上限的退避重试，不再在一次失败或原来的五秒窗口后永久放弃喵~
- 已挂载的语言 Provider 缓存不会仅因方法包装自动更新，必要时按版本和目标语言只刷新一次；无法持久记录刷新标记时推迟刷新，避免重载循环喵~
- 关闭强制中文时还原 navigator、语言 API 与此前语言设置，保留用户后来手动选择的语言；切换开关会取消过期请求，不在失败响应或无法备份原语言时覆盖设置喵~
- 新增十三项隔离回归，保留高级提示词恢复、纯 API 本地上下文、供应商、微信与现有更新源；三平台完整测试通过后由 GitHub Actions 构建六项正式安装资产喵~
- 已排查 Ultra 与完全访问的两条原生确认路径，本版不加入 Ultra 自动提权、全局自动确认或永久跳过原生授权的行为喵~

## 1.0.11 - 2026-09-06

- 高级提示词在每次启动前、所有供应商/MCP/上下文配置写入结束后检查；托管文件或引用缺失时自动补建，优先恢复本地 last-good 备份和用户已保存正文，首次启用且无正文时才创建基础模板；按当前 CODEX_HOME 写入绝对引用，支持非默认配置目录喵~
- 保存无关 Agent 能力不再覆盖直接编辑的提示词；显式编辑正文才同步新文本，关闭开关只移除托管引用而不删除文件，重新启用可继续使用原文；补齐局部设置更新的两个 instructions 字段并保护自定义外部引用、无效/不可读配置喵~
- 修复旧 `guardianv2` 非布尔、非表值造成的配置解析问题，保留仍受新版 Codex 支持的布尔值与结构化 Guardian 设置，不再无条件删除该功能配置喵~
- 适配 Codex 26.901 资源布局：资源读取合并与缓存、缺失模块冷却和限次重试、dispatcher 单重试定时器、终端失败 Promise 恢复及扩展自身 DOM 变更过滤喵~
- 插件过滤按完整谓词结构识别而不依赖压缩变量名，保留搜索与 thisArg 语义；托管市场迁为 `alunixa-x-curated` 并同步 manifest、配置和禁用选择，不覆盖用户自建市场喵~
- 纯 API 会话恢复规范化继承的官方 provider；兼容 `model/list` 与原生 `serviceTiers`，Fast UI 与请求能力判定保持一致喵~
- 会话删除/撤销同步 `session_index.jsonl` 和 `local_thread_catalog`，支持仅索引任务、新状态数据库、多库失败恢复与 revision，保留无关损坏行及同 ID 远端记录喵~
- 微信按 thread/turn 过滤回复与实时进度，重放响应前通知，只有目标 turn 正式完成才返回；区分 final_answer、失败/中断及可重试错误，固定绝对读取期限，Windows 子进程不弹控制台，同时保留 v1.0.8/v1.0.9 的 CLI 回退、实时进度、节流和脱敏喵~
- Dream Skin 支持等价配置路径，并独立保留失效旧路径的备份；修复 macOS updater 跨架构测试数据与 provider-sync 非确定性并发测试，不放宽产品规则或跳过失败测试喵~
- 正式代码与六类安装文件只发布到 `Alunixa-Code/Alunixa-X`，增加仓库目标保护；三平台运行完整前端/Rust 测试后锁文件构建，保留 Windows 独立官方 CLI 的纯 API 原生窗口端到端验证，发行说明自动包含提交、Actions 来源和六项 SHA-256 喵~
- 本版以 Alunixa X v1.0.10 为基线整合兼容修复，不包含旧仓库的桥接更新器或固定 v1.0.6 迁移资产；本地上下文、仪表盘统计、现有品牌/图标和更新源保持不变喵~
