...
async fn main() -> anyhow::Result<()> {
    let bind = env::var("ALUNIXA_X_MOBILE_RELAY_BIND")
        .ok()
    ...
    println!("Alunixa X mobile relay listening on ws://{local_addr}");
    println!(
...
async fn handle_http_connection(
    ...
    let (status, content_type, body) = match path {
        ...
        "/health" => (
            ...
            serde_json::json!({
                "status": "ok",
                "service": "alunixa-x-mobile-relay",
...
<head>
  ...
  <title>Alunixa X Mobile Relay</title>
  <style>
...
<body>
<div class="app">
  <header class="topbar"><strong class="title">Alunixa X</strong><span id="status" class="status">待连接</span></header>
  <main class="layout">
...
async function ensureAppServer() {
  ...
  await rpcRaw("initialize", { clientInfo: { name: "Alunixa X Mobile Relay", version: "1.0.0" }, capabilities: { experimentalApi: true } });
}
...
async function sendMessage(threadId, text, skipResume = false) {
  ...
  return await rpc("turn/start", { threadId, clientUserMessageId: `alunixa-x-mobile-${Date.now()}`, input: [{ type: "text", text }] });
}
