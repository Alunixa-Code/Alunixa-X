...
describe("model-windows helpers", () => {
  ...
  it("退役的实验性上下文不再出现在界面、默认设置和文案中", () => {
    const source = fs.readFileSync(new URL("./App.tsx", import.meta.url), "utf8");
    const dictionary = fs.readFileSync(new URL("./i18n-en.ts", import.meta.url), "utf8");
    assert.doesNotMatch(source, /codexAppExperimentalContext|实验性上下文|features\.token_budget/);
    assert.doesNotMatch(dictionary, /"实验性上下文"|features\.context_management\.experimental_mode/);
  });
