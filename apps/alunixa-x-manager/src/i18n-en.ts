...
export const EN_PLAIN: Record<string, string> = {
  ...
  "默认关闭。开启后写入 features.context_management.experimental_mode = true。纯 API / 未登录时自动使用本地兼容模式：原生上下文窗口管理 + 本地笔记与历史检索，无需 ChatGPT 登录；官方云端模式要求 ChatGPT Plus、Pro 或 Pro Lite。兼容模式不是传统摘要压缩，不改动原有自动压缩 Token 阈值，默认额外预留 2048 Token 保存笔记（不突破模型上限）。关闭后恢复托管参数，保留笔记。开关自动保存，需通过 Alunixa X 重启 Codex 后生效，并使用支持 token_budget 的新版 Codex。":
    "Off by default. Writes features.context_management.experimental_mode = true. API-only or signed-out sessions automatically use local compatibility mode: native context-window management plus local notes and history search, without ChatGPT login. The official cloud mode requires ChatGPT Plus, Pro, or Pro Lite. Local mode replaces summary compaction with window rollover, keeps your configured token threshold, and reserves an extra 2048 tokens for notes by default without exceeding the model limit. Disabling restores managed settings and retains notes. Saves automatically; restart Codex through Alunixa X with a version supporting token_budget.",
