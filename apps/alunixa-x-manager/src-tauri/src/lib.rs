...
const TRAY_ID: &str = "alunixa_x_tray";
...
pub fn run() {
    ...
    let app_result = tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::launch_alunixa_x,
            commands::restart_alunixa_x,
            commands::load_settings,
            commands::save_settings,
            commands::import_wallpaper_media,
            commands::inspect_wallpaper,
            commands::repair_codex_feature_config,
            ...
            commands::dashboard_usage_analytics,
            ...
            commands::preview_rollout_image_cleanup,
            commands::apply_rollout_image_cleanup,
            commands::restore_rollout_image_cleanup,
...
pub fn handle_dream_skin_url(url: &str) -> bool {
    ...
    match alunixa_x_core::dream_skin_community::save_pending_community_link(url) {
        Ok(version_id) => {
            let _ = alunixa_x_core::diagnostic_log::append_diagnostic_log(
...
pub fn focus_existing_manager_window() {
    #[cfg(windows)]
    {
        ...
        for process in alunixa_x_core::windows_enumerate_processes() {
            if process.process_id == current_process_id {
                continue;
            }
            if process
                .exe_file
                .eq_ignore_ascii_case("alunixa-x-manager.exe")
            {
                let _ = alunixa_x_core::windows_activate_process_window(process.process_id);
                break;
            }
        }
    }
...
fn acquire_single_instance_guard() -> Option<alunixa_x_core::ports::LoopbackPortGuard> {
    match alunixa_x_core::ports::acquire_resilient_loopback_port_guard(
        alunixa_x_core::ports::manager_guard_port(),
    ) {
        Ok(guard) => {
            if let Some(fallback_lock_path) = guard.fallback_path() {
                let _ = alunixa_x_core::diagnostic_log::append_diagnostic_log(
                    ...
                    serde_json::json!({
                        "requested_guard_port": alunixa_x_core::ports::manager_guard_port(),
            ...
            ) =>
        {
            let _ = alunixa_x_core::diagnostic_log::append_diagnostic_log(
                ...
                serde_json::json!({
                    "guard_port": alunixa_x_core::ports::manager_guard_port()
                }),
            );
        ...
        Err(error) => {
            ...
            match std::net::TcpListener::bind(("127.0.0.1", 0)) {
                Ok(listener) => Some(alunixa_x_core::ports::LoopbackPortGuard::listener(listener)),
                Err(fallback_error) => {
                    let _ = alunixa_x_core::diagnostic_log::append_diagnostic_log(
