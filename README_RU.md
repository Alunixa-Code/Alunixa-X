...
Начиная с **v1.0.25**, профиль с несколькими моделями больше не копирует окно стартовой модели в корневой `config.toml`. Параметры `model_context_window` и `model_auto_compact_token_limit` являются глобальными переопределениями Codex. При старой схеме после переключения другая модель продолжала получать ограничения стартовой модели, поэтому значения в менеджере и внутри Codex могли различаться.

В режиме нескольких моделей окно контекста и порог автосжатия хранятся только в `model_catalog_json`, отдельно для каждой модели. В корневой конфигурации остаются имя стартовой модели, провайдер и ссылка на каталог; редактирование другой модели не выбирает её автоматически. Обычные одномодельные профили и режим Pure API по-прежнему могут использовать корневые ограничения.
...
- При отключении автосжатия управляемый порог этой модели не записывается в каталог.
