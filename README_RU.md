...
| Оформление | DreamSkin, локальные и ZIP-темы, видеообои, GIF/APNG и проекты Wallpaper Engine |
...
- Только для нативных обоев Scene/Web — Windows и отдельно установленный Wallpaper Engine.
...
Используйте **v1.0.24 или новее**: в v1.0.22/v1.0.23 основной лаунчер не обрабатывал запросы обоев и возвращал `Unknown bridge path`, хотя изолированный тест проходил. Теперь оба лаунчера и тест используют одну точку подключения моста; русский интерфейс сохранён.

Если нужен только первоначальный фон из изображения, импортируйте PNG/JPEG или другую статичную картинку. Для этого Wallpaper Engine не нужен и не запускается; видео и сцены включать необязательно.

### Настройка

1. Откройте **Оформление → Живые обои**; те же элементы доступны в настройках.
2. Нажмите **Импортировать файл обоев** для картинки/анимации/видео либо **Выбрать папку проекта** для Wallpaper Engine.
3. Включите обои, задайте непрозрачность и масштабирование; при необходимости настройте звук и паузу.
4. Нажмите **Сохранить настройки**.
5. При следующем запуске Codex используйте обновлённый Alunixa X Launch.

Предпросмотр в менеджере всегда без звука. Сохранение не меняет и не перезапускает текущее окно Codex. **Сбросить фон** сбрасывает только настройки обоев, а не провайдеров, задач или DreamSkin.

### Что поддерживается

| Формат | Windows | macOS | Особенности |
| --- | --- | --- | --- |
| PNG, JPEG, BMP, статический WebP | Да | Да | Исходное неподвижное изображение |
| GIF, APNG, анимированный WebP | Да | Да | Анимация сохраняется, если она есть в файле |
| MP4, M4V, WebM, MOV, OGV | Да | Да | До 2 GiB; фактическое декодирование зависит от кодека |
| Wallpaper Engine Video | Да | Да | Воспроизводится видео из `project.json` |
| Wallpaper Engine Scene / Web | Да | Нет | Нужен установленный Wallpaper Engine |
| Wallpaper Engine Application | Не запускается | Не запускается | Произвольные исполняемые обои не поддерживаются |

Рекомендуемые видео — MP4/H.264 или WebM. Расширение `.mp4` само по себе не гарантирует совместимость кодека. Обычный PNG не становится анимированным: для этого нужен APNG или другой анимированный формат.

### Как выбрать проект Wallpaper Engine

Выберите **одну папку, содержащую `project.json`**, например каталог отдельного Workshop-проекта с `scene.pkg` и `preview.jpg`. Не выбирайте корень Steam, всю библиотеку `workshop/content/431960` или папку установки Wallpaper Engine.

Для Scene/Web программа может найти `wallpaper64.exe` / `wallpaper32.exe` по библиотеке Steam; при необходимости укажите исполняемый файл вручную. Сцены с ресурсами внутри `scene.pkg` распознаются.

Scene/Web рендерятся Wallpaper Engine в окне с уникальным именем, которое с самого запуска находится за пределами всех мониторов. Оно не должно появляться на рабочем столе или панели задач и не активируется. Окно не сворачивается, чтобы захват продолжал получать живые кадры. В Codex передаётся только его изображение; рабочий стол и чужие окна не захватываются. Глобальная команда остановки движка не отправляется; при завершении закрывается только собственное окно.

Нативный захват ограничен **1280×720 и 24 FPS**. Пауза останавливает показ в Codex, но не обязательно рендеринг движка. Звук сцены управляется свойствами отдельного окна Wallpaper Engine; системный звук не входит в видеопоток.

Веб-проект выполняется в среде Wallpaper Engine, а не в iframe Codex. Его сеть и возможности хоста обслуживает движок. Alunixa X не превращает превью `preview.jpg` в «работающую сцену» при ошибке захвата.

Импортированные медиа копируются без перекодирования в папку `wallpapers` состояния приложения; исходный файл не меняется. Проекты WE остаются привязаны к исходному пути: после переноса проекта выберите папку заново.

Большие изображения и видео передаются через ограниченный мост в дисковый `File → Blob URL`, без встраивания всего видео в Base64 и без ослабления CSP Codex. Подробности и отдельные тесты — в [руководстве по обоям](docs/wallpapers.md).
...
Локальная обработка не означает полностью автономную работу: запросы к моделям отправляются вашим провайдерам, обновления и каталоги используют сеть, WeChat и Remote Control обращаются к своим сервисам, а веб-обои работают в сетевой среде Wallpaper Engine.
...
Проверьте наличие `project.json` именно в выбранном проекте, тип Video/Scene/Web, Windows и путь к движку. На macOS используйте изображения, анимацию или Video. Миниатюра проекта не подтверждает работающий захват.
